...
//+------------------------------------------------------------------+
//|                                            Donchian Breakout.mq4 |
//|                             Copyright © 2010 Boston Technologies |
...
#property copyright "Boston Technologies"
#property link      "www.bostontechnologies.com"
...
int start()
{
   ...
   if( LAST_BAR == iHighest( Symbol(), Period(), MODE_HIGH, Lookback, LAST_BAR) ) {
      ...
      if( lastTradeTime != Time[ THIS_BAR] && NoOpenPositionsExist( ORDER_COMMENT ) && LastTradeWas( SHORT ) && CheckTime( StartTime, EndTime) ) {
   ...
   if( LAST_BAR == iLowest( Symbol(), Period(), MODE_LOW, Lookback, LAST_BAR) ) {
      ...
      if( lastTradeTime != Time[ THIS_BAR] && NoOpenPositionsExist( ORDER_COMMENT ) && LastTradeWas( LONG ) && CheckTime( StartTime, EndTime)  ) {
...
bool LastTradeWas( int direction ) {
