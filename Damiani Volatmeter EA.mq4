...
bool deleteonopposite=false;
bool stoporders=false;
bool limitorders=false;
int gap=20;

extern string entrylogics="Entry Settings";
...
extern int filtermode=0;                 // 0=don't use, 1=use
extern int timeframe2=60;
extern int visatr2=13;
extern int visstd2=20;
extern int sedatr2=40;
extern int sedstd2=100;
extern double thresholdlevel2=1.4;
...
*/
   ...
   //if(time!=Time[0] || shift==0){
      ...
      double dvgray=iCustom(NULL,timeframe,"Damiani_volatmeter",visatr,visstd,sedatr,sedstd,thresholdlevel,0,shift);
      double dvgeen=iCustom(NULL,timeframe,"Damiani_volatmeter",visatr,visstd,sedatr,sedstd,thresholdlevel,2,shift);
      double dvgraya=iCustom(NULL,timeframe,"Damiani_volatmeter",visatr,visstd,sedatr,sedstd,thresholdlevel,0,shift+1);
      double dvgeena=iCustom(NULL,timeframe,"Damiani_volatmeter",visatr,visstd,sedatr,sedstd,thresholdlevel,2,shift+1);
      
      double dvgraytf=iCustom(NULL,timeframe2,"Damiani_volatmeter",visatr2,visstd2,sedatr2,sedstd2,thresholdlevel2,0,shift);
      double dvgeentf=iCustom(NULL,timeframe2,"Damiani_volatmeter",visatr2,visstd2,sedatr2,sedstd2,thresholdlevel2,2,shift);
      double dvgrayatf=iCustom(NULL,timeframe2,"Damiani_volatmeter",visatr2,visstd2,sedatr2,sedstd2,thresholdlevel2,0,shift+1);
      double dvgeenatf=iCustom(NULL,timeframe2,"Damiani_volatmeter",visatr2,visstd2,sedatr2,sedstd2,thresholdlevel2,2,shift+1);
      //time=Time[0];
   //}
   bool buy=false;
   bool sell=false;
   ...
   if(
   dvgeena<dvgraya && dvgeen>dvgray
   && (filtermode==1 && dvgeenatf<dvgrayatf && dvgeentf>dvgraytf) || filtermode==0
   && ma>maa
...
*/
   ...
   if(buy && tpb<tradesperbar){
      if(deleteonopposite){delete(OP_SELLSTOP);delete(OP_SELLLIMIT);}
      ...
      if(ecn==false){
         ...
         if(stoporders){
            RefreshRates();
            ticket=open(OP_BUYSTOP,ilots,Ask+gap*pt,stoploss,takeprofit,expire,Blue);
         }
         if(limitorders){
            RefreshRates();
            ticket=open(OP_BUYLIMIT,ilots,Bid-gap*pt,stoploss,takeprofit,expire,Blue);
         }
      }
      if(ecn){
         if(instantorders){
         ...
         if(stoporders){
            RefreshRates();
            ticket=open(OP_BUYSTOP,ilots,Ask+gap*pt,0,0,expire,Blue);
         }
         if(limitorders){
            RefreshRates();
            ticket=open(OP_BUYLIMIT,ilots,Bid-gap*pt,0,0,expire,Blue);
         }
   ...
   if(sell && tps<tradesperbar){
      if(deleteonopposite){delete(OP_BUYSTOP);delete(OP_BUYLIMIT);}
      /*if(martingale)ilots=mlots;else*/ ilots=lots;
      if(ecn==false){
         if(instantorders){
            while(ticket<=0 && number<tries){
         ...
         if(stoporders){
            RefreshRates();
            ticket=open(OP_SELLSTOP,ilots,Bid-gap*pt,stoploss,takeprofit,expire,Red);
         }
         if(limitorders){
            RefreshRates();
            ticket=open(OP_SELLLIMIT,ilots,Ask+gap*pt,stoploss,takeprofit,expire,Red);
         }
      }
      if(ecn){
         if(instantorders){
         ...
         if(stoporders){
            RefreshRates();
            ticket=open(OP_SELLSTOP,ilots,Bid-gap*pt,0,0,expire,Red);
         }
         if(limitorders){
            RefreshRates();
            ticket=open(OP_SELLLIMIT,ilots,Ask+gap*pt,0,0,expire,Red);
         }
