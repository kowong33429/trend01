      ...
      if(atr>atr_ma)
        {
         if(bb_stop_trend_cur == 1.0)
           {
            if(true)//volume>volumeAvg
              {
               if(iClose(NULL,0,1)>ma10)
                 {
                  if(iClose(NULL,0,1)-iCustom(NULL,0,"bb-stops-v2-indicator",3,1) < 2.5*atr)
                    {
                     if(iClose(NULL,0,1)-iCustom(NULL,0,"bb-stops-v2-indicator",3,1)>10)
                       {
                        OrderSend(_Symbol,OP_BUY,0.05,Ask,3,iCustom(NULL,0,"bb-stops-v2-indicator",3,1)-2,0,"test",1111,0,clrGreen);
                       }
                     else
                       {
               ...
               if(true)//volume>volumeAvg
                 {
                  if(iClose(NULL,0,1)<ma10)
                    {
                     if(iCustom(NULL,0,"bb-stops-v2-indicator",4,1)-iClose(NULL,0,1) < 2.5*atr)
                       {
                        if(iCustom(NULL,0,"bb-stops-v2-indicator",4,1)-iClose(NULL,0,1)>10)
                          {
                           OrderSend(_Symbol,OP_SELL,0.05,Bid,3,iCustom(NULL,0,"bb-stops-v2-indicator",4,1)+2,0,"test",1111,0,clrRed);
                          }
                        else
                          {
         ...
         if(OrderProfit()>0)
           {
            ...
            if(bb_stop_trend_cur == 1.0)
              {
               if(OrderProfit()>0 && OrderOpenPrice()+5)
                 {
                  OrderModify(OrderTicket(),OrderOpenPrice(),iCustom(NULL,0,"bb-stops-v2-indicator",3,1)-2,OrderTakeProfit(),0,Blue);
                 }
            ...
            if(bb_stop_trend_cur == -1.0)
              {
               if(OrderProfit()>0 && OrderOpenPrice()-5)
                 {
                  OrderModify(OrderTicket(),OrderOpenPrice(),iCustom(NULL,0,"bb-stops-v2-indicator",4,1)+2,OrderTakeProfit(),0,Red);
                 }
