...
//fresh signal change trend
               ...
               if(iClose(NULL,0,1)>ma10)
                 {
                  if(iClose(NULL,0,1)-iCustom(NULL,0,"bb-stops-v2-indicator",3,1)<10)
                    {
                     OrderSend(_Symbol,OP_BUY,0.1,Ask,3,iCustom(NULL,0,"bb-stops-v2-indicator",3,1)-2,0,"JubuBot",1111,0,clrGreen);
                    }
                 }
              }
           }
                  ...
                  if(iClose(NULL,0,1)<ma10)
                    {
                     if(iCustom(NULL,0,"bb-stops-v2-indicator",4,1)-iClose(NULL,0,1)<10)
                       {
                        OrderSend(_Symbol,OP_SELL,0.1,Bid,3,iCustom(NULL,0,"bb-stops-v2-indicator",4,1)+2,0,"JubuBot",1111,0,clrRed);
                       }
                    }
                 }
              }
   ...
   for(int i=OrdersTotal()-1; i>=0; i--)
     {
      if(OrderSelect(i,SELECT_BY_POS,MODE_TRADES) && OrderType() == OP_BUY && OrderComment() == "JubuBot" && TimeCurrent() - OrderOpenTime() > 3600)
        {
         if(OrderProfit()>0)
           {
      ...
      if(OrderSelect(i,SELECT_BY_POS,MODE_TRADES) && OrderType() == OP_SELL && OrderComment() == "JubuBot" && TimeCurrent() - OrderOpenTime() > 3600)
        {
