...
bool check_slope_macd(string trend)
{
   ...
   if(trend == "buy")
   {
      ...
      {
         if(iMACD(_Symbol,PERIOD_M15,12,26,9,PRICE_CLOSE,MODE_MAIN,i)>iMACD(_Symbol,PERIOD_M15,12,26,9,PRICE_CLOSE,MODE_MAIN,i+1))
         {
            count++;
         }
      }
...
//---
   ...
   if(check_alligator(jaw_h1,teeth_h1,gatorlips_h1) == check_alligator(jaw_h4,teeth_h4,gatorlips_h4))
   {
      ...
      {
         //check macd buy
         if(check_slope_macd("buy"))
         {
            //check previous candle is green
            if(iClose(_Symbol,PERIOD_M15,1)>iOpen(_Symbol,PERIOD_M15,1) && MathAbs(iClose(_Symbol,PERIOD_M15,1)-iOpen(_Symbol,PERIOD_M15,1))>10*Point)
            {
               if(iClose(_Symbol,PERIOD_M15,1)>teeth_h1 && iClose(_Symbol,PERIOD_M15,1)-teeth_h1<100*Point)
               {
      ...
      //sell
         //check macd sell
         if(check_slope_macd("sell"))
         {
            //check previous candle is green
            if(iClose(_Symbol,PERIOD_M15,1)<iOpen(_Symbol,PERIOD_M15,1) && MathAbs(iClose(_Symbol,PERIOD_M15,1)-iOpen(_Symbol,PERIOD_M15,1))>10*Point)
            {
               if(iClose(_Symbol,PERIOD_M15,1)<teeth_h1 && teeth_h1-iClose(_Symbol,PERIOD_M15,1)<100*Point)
               {
